...
const AttemptBlindQuiz = ({ match }) => {
		...
		(event) => {
			if (event.keyCode === 32) {
				...
				// For Quiz not accessible
					...
					speak('Listening Started. Say Instructions for further guide.');
					resetTranscript();
				}
			}
		},
	...
	useEffect(() => {
		...
		const temp = [
			...
			},
			{
				...
				callback: () => {
					speak(
						'Use the following voice commands to interact with the application. Start Quiz to initialize the quiz. Repeat the current question command to repeat the focused question. Select option number command for choosing the correct option. Next question to move to the next consecutive question and Previous Question to move to the previous consecutive question.'
					);
					resetTranscript();
				},
			},
	...
	//select options and send to database
